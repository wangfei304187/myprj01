...
public class ZkUtils
{
    ...
    /**
     * Sorts a list of ZooKeeper-nodes based on the sequential pattern set by ZooKeeper, which is {@code (prefix)(delimiter)(sequence number)}.
     * <p>
     * When this method completes, {@code items} will be sorted according to the sequence number, with the lowest sequence number occurring first.
     *
     * @param items
     *            the items to sort
     * @param sequenceDelimiter
     *            the delimiter separating prefix elements from sequence numbers
     */
    ...
    /**
     * Sorts a list of ZooKeeper-nodes based on the sequential pattern set by ZooKeeper, which is {@code (prefix)(delimiter)(sequence number)}, in reverse order.
     * <p>
     * When this method completes, {@code items} will be sorted according to the sequence number, with the highest sequence number occurring first.
     *
     * @param items
     *            the items to sort
     * @param sequenceDelimiter
     ...
     * @param node
     *            the node to parse
     * @param sequenceStartDelimiter
     *            the delimiter separating a node prefix from its sequence number
    ...
    {
        String sequenceStr = ZkUtils.parseSequenceString(node, sequenceStartDelimiter);
     ...
     * <p>
     * This method is all-inclusive--if any element in the node list starts with any of the given prefixes, then it is included in the result.
     *
     * @param nodes
     *            the nodes to filter
     * @param prefixes
     ...
     * <p>
     * If the element has already been deleted from ZooKeeper, then a NoNode Exception is normally thrown by ZooKeeper. This method suppresses those NoNode exceptions .
     *
     * @param zk
     ...
     *            the node to delete
     * @param version
     *            the version of that node to remove
     * @return true if the node was deleted
     * @throws KeeperException
     *             if some issue with the ZooKeeper server occurs
     * @throws InterruptedException
     *             if some communication error happens
     ...
     * <p>
     * If any element has already been deleted from ZooKeeper, then a NoNode Exception is normally thrown by ZooKeeper. This method suppresses those exceptions, while allowing through all other kinds.
     * <p>
     * If ZooKeeper Exception which is <i>not</i> of the type NoNode, then it will be thrown immediately, and any elements which had not yet been deleted will not be deleted by this method.
     *
     * @param zk
     *            the ZooKeeper client to use
     * @param nodesToDelete
     *            the nodes to delete
     * @param version
     *            the version of the nodes to remove
     * @throws KeeperException
     *             if some issue with the ZooKeeper server which is <i>not</i> of type NoNode occurs.
     * @throws InterruptedException
     *             if some communication error happens between the ZooKeeper client and server
     */
     ...
     * If any element has already been deleted from ZooKeeper, then a NoNode Exception is normally thrown by ZooKeeper. This method suppresses those exceptions, while allowing through all other kinds.
     * <p>
     * If ZooKeeper Exception which is <i>not</i> of the type NoNode, then it will be thrown immediately, and any elements (such as additional children) which had not yet been deleted will not be
     * deleted by this method.
     *
     * @param zk
     *            the ZooKeeper client to use
     * @param nodeToDelete
     *            the node to recursively delete
     * @param version
     *            the version of the nodes to remove
     * @throws KeeperException
     *             if some issue with the ZooKeeper server which is <i>not</i> of type NoNode occurs.
    ...
    public static void recursiveSafeDelete(ZooKeeper zk, String nodeToDelete, int version) throws KeeperException, InterruptedException
    {
        try
        {
            ...
            {
                ZkUtils.recursiveSafeDelete(zk, nodeToDelete + "/" + child, version);
            }
            // delete this node
            ZkUtils.safeDelete(zk, nodeToDelete, version);
        }
        catch (KeeperException ke)
        {
            if (ke.code() != KeeperException.Code.NONODE)
     ...
     * <p>
     * If a node already exists, ZooKeeper may throw a KeeperException of type {@code KeeperException.Code.NODEEXISTS}. This method suppresses that exception, allow creates of potentially contentious
     * nodes to remain atomic.
     * <p>
     * If the node already exists, the data will <i>not</i> be set to that node. This reflects the fact that a create-and-set operation is not atomic, and may result in a data race. Instead, this
     * method will quietly return.
     * <p>
     * If any other type of KeeperException is thrown, that exception will be propogated up the stack.
     * <p>
     * Note: If the CreateMode of the node of interest is sequential, then this method is unnecessary, as ZooKeeper will never throw a NODEEXISTS exception for modes of those types.
     *
     * @param zk
     *            the ZooKeeper client to use
     * @param nodeToCreate
     *            the node to create
     * @param data
     *            the data to set on the newly created node.
     * @param privileges
     *            the privileges to associate with that node
     * @param createMode
     *            the Mode that node will possess
     * @return the name of the newly qualified node
     * @throws KeeperException
     *             if an Error occurs on the ZooKeeper server which is <i>not</i> of type NODEEXISTS
     * @throws InterruptedException
     *             if a communication error between the client and server occurs.
     */
     ...
     * <p>
     * Because it is possible that the node under request does not exist, it is possible that a KeeperException.NONODE exception will be thrown. This method swallows NONODE exceptions and returns an
     * empty byte[] when that node does not exist.
     *
     * @param zk
     *            the ZooKeeper client
     * @param node
     *            the node to get data for
     * @param watcher
     *            any watcher to attach to this node, if it exists
     * @param stat
     *            a stat object for use in the getData call
     ...
     *
     * @throws InterruptedException
     *             if communication between the ZooKeeper client and servers are broken
     * @throws KeeperException
     *             if there is a problem getting Data from the ZooKeeper server that <i>doesn't</i> have
     ...
     *            the node to get data for
     * @param watch
     *            true if the default watcher is to be attached to this node's data
     * @param stat
     *            a stat object for use in the getData call
     * @return the byte[] information contained in that node, if that node exists. If {@code node} does not exist,
     *         then this returns an empty byte[]
    ...
    public static String recursiveSafeCreate(ZooKeeper zk, String node, byte[] data, List<ACL> privileges, CreateMode createMode) throws KeeperException, InterruptedException
    {
        ...
        {
            return node; // can't create any further
        }
        else
        {
            ...
            ZkUtils.recursiveSafeCreate(zk, parent, data, privileges, createMode);

            // create this node now
            return ZkUtils.safeCreate(zk, node, data, privileges, createMode);
        }
    }
    ...
    private static class SequenceComparator implements Comparator<String>
    {
        ...
        public int compare(String child1, String child2)
        {
            ...
            else if (childOneSeqNbr > childTwoSeqNbr)
            {
                return 1;
            }
            else
