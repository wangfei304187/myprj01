...
public class CubeTexture3D implements GLEventListener
{
    ...
    public void init(GLAutoDrawable drawable)
    {
        ...
        try
        {
            ...
            // TextureData[] textures = new TextureData[depth];
            // for (int k = 0; k < textures.length; k++) {
            // TextureData textureData = new TextureData(gl.getGLProfile(), GL2ES2.GL_LUMINANCE,
            // width, height, 0, GL2ES2.GL_LUMINANCE, GL4.GL_BYTE, false,
            // false, false, null, null);
            // byte[] pixelData = du.getPixels8(axialList.get(k));
            // textureData.setBuffer(Buffers.newDirectByteBuffer(pixelData));
            // textures[k] = textureData;
            // }

            List<BufferedImage> jpgList = loadDataJpg();

            // TextureData textureData = null;
            // gl.glBindTexture(GL2.GL_TEXTURE_3D, texture);
            // gl.glActiveTexture(GL4.GL_TEXTURE0);
            gl.glBindTexture(GL2ES3.GL_TEXTURE_2D_ARRAY, texture);
            System.out.println("loadTextureArray func 0: " + gl.glGetError());

            // gl.glTexParameteri(GL2.GL_TEXTURE_3D, GL.GL_TEXTURE_MIN_FILTER,
            // GL.GL_NEAREST);
            // gl.glTexParameteri(GL2.GL_TEXTURE_3D, GL.GL_TEXTURE_MAG_FILTER,
            // GL.GL_LINEAR);
            // gl.glTexImage3D(GL2.GL_TEXTURE_3D, 0, GL.GL_R8,
            // width, height, depth, 0,
            // GL.GL_R8, GL.GL_UNSIGNED_BYTE, textureData.getBuffer());
            // gl.glGenerateMipmap(GL2.GL_TEXTURE_3D);
            gl.glTexStorage3D(GL2ES3.GL_TEXTURE_2D_ARRAY, 1, GL.GL_RGB8, width, height, depth);
            //gl.glTexImage3D(GL2ES3.GL_TEXTURE_2D_ARRAY, 1, GL.GL_RGB, width, height, depth, 0, GL.GL_RGB, GL.GL_UNSIGNED_BYTE, null);
            ...
            for (int k = 0; k < 51; k++)
            {
                ...
                BufferedImage bi = jpgList.get(k);
