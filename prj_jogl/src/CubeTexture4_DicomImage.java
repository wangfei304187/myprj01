...
import com.kdevn.image.util.ImageDisplayUtils;
import com.kdevn.image.util.ImageUtils;
import com.kdevn.image.util.PixelUtils;

public class CubeTexture4_DicomImage implements GLEventListener {
	...
	public void display(GLAutoDrawable drawable) {
		...
		// gl.glBindTexture(GL4.GL_TEXTURE_3D, texture);
	...
	public void init(GLAutoDrawable drawable) {
		...
		try {
			...
			TextureData textureData = new TextureData(gl.getGLProfile(),
					GL2ES2.GL_LUMINANCE, width, height, 0, GL2ES2.GL_LUMINANCE,
					GL4.GL_UNSIGNED_BYTE, false, false, false, null, null);
			
			// dicom image --> pixels8 --> textureData
			DicomObject dcmObj = axialList.get(30);
			byte[] pixelData = du.getPixels8(dcmObj);
			int cols = du.getColumns(dcmObj);
	        int rows = du.getRows(dcmObj);
	        int rescaleSlope = du.getRescaleSlope(dcmObj);
	        int rescaleIntercept = du.getRescaleIntercept(dcmObj);
	        int wc = du.getWC(dcmObj);
	        int ww = du.getWW(dcmObj);
	        short[] overlay = du.getOverlayData(dcmObj);
	        ImageDisplayUtils u = new ImageDisplayUtils();
	        u.setWC(wc);
	        u.setWW(ww);
	        byte[] pixels8 = u.create8BitImage(cols, rows, pixelData, rescaleSlope, rescaleIntercept, overlay, "LINEAR");
			
	        textureData.setBuffer(Buffers.newDirectByteBuffer(pixels8));
			...
			System.out.println("loadTextureArray func 0: " + gl.glGetError());
			 ...
			 gl.glTexImage2D(GL.GL_TEXTURE_2D, 0, GL.GL_LUMINANCE,
					textureData.getWidth(), textureData.getHeight(), 0,
					GL.GL_LUMINANCE, GL4.GL_UNSIGNED_BYTE, textureData.getBuffer());
			gl.glGenerateMipmap(GL.GL_TEXTURE_2D);
	...
	public static void main(String[] args) {
		...
		CubeTexture4_DicomImage app = new CubeTexture4_DicomImage();
		app.showFrame();
	}
	...
	public CubeTexture4_DicomImage() {
		super();
		topPnl = new JPanel();
